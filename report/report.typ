...
游戏的显示模块，负责接收一个坐标对应的区块种类及具体区块内坐标信息，由此与 IP 核交互，得到 RGB 显示信息并传出。
...
在初步设计时，为对 LOGIC 模块进行单独仿真，这里在其中初始化了如下地图，其中“`#`”表示墙，“` `”表示空地，“`$`”表示箱子，“`.`”表示目标位置，“`@`”表示角色。
...
#figurex("./assets/logic_tb-1.png", "LOGIC 模块仿真", width: 100%)
...
#figurex("./assets/logic_tb-2.png", "逻辑正常", width: 110%)

操作玩家推箱子直至通关，发现当得分为 4 之后，触发通关条件，分数清零，玩家位置重置，此时发生了过关的关卡重置，这说明过关逻辑正常，以此完成了对 LOGIC 模块的单独仿真测试。
...
#figurex("./assets/play-1.jpg", "游戏初始界面", width: 90%)

按下空格进入游戏，如下为第一关初始情况。

#figurex("./assets/play-2.jpg", "第一关初始情况", width: 90%)

此时关卡显示为 1，得分显示为 0。

#figurex("./assets/play-3.jpg", "分数显示 0", width: 90%)

通过键盘操作通关，游玩片刻后，仅差一个箱子到位即可通关。

#figurex("./assets/play-4.jpg", "仅差一个箱子到位即可通关", width: 90%)

此时分数显示为 3。

#figurex("./assets/play-5.jpg", "分数显示 3", width: 90%)

推箱子通关后，进入第二关。

#figurex("./assets/play-6.jpg", "第二关", width: 90%)

其初始化为关卡显示为 2，得分显示为 0。

#figurex("./assets/play-7.jpg", "关卡显示 2", width: 90%)

随着进一步的游玩，游戏推进到了最后一关第五关，此时已经有些难度了。

#figurex("./assets/play-8.jpg", "第五关", width: 90%)

关卡显示为 5，得分显示为 1。

#figurex("./assets/play-9.jpg", "关卡显示 5", width: 90%)
